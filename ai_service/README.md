...
predict.py      (latest 30 days → 7-day forecast → risk classification → recommendations)
        │
        ▼
api.py (FastAPI :8080)  ←—axios——  server/ (Express :8080 /api/v1/forecast/*)  ←——  client/ (React)
```
...
```bash
curl http://localhost 8080/forecast/all
curl http://localhost 8080/alerts
curl http://localhost 8080/forecast/O%2B
```
...
```bash
ENABLE_AUTO_RETRAIN=true RETRAIN_INTERVAL_HOURS=168 uvicorn api:app --port 8080
```
