...
const adminNavLinks = [
  { to: "/admin", icon: MdHome, label: "Home" },
  { to: "/help", icon: MdSupportAgent, label: "Need Help" },
  { to: "/about", icon: MdInfoOutline, label: "About Us" },
  { to: "/contact", icon: MdPhone, label: "Contact Us" },
];
...
function getDisplayName(user) {
  ...
  if (role === "hospital") return user?.hospitalName ?? user?.name;
  if (role === "organisation") return user?.organisationName ?? user?.name;
  return user?.name;
}
      ...
      <div className="flex flex-col gap-0.5 w-4">
        {[
          ...
          <span
            ...
            style={{ transform: s.transform, opacity: s.opacity }}
          />
        ))}
      </div>
    </button>
...
export default function Navbar() {
  ...
  // Admin ko alag nav links dikhao
  const navLinks = role === "admin" ? adminNavLinks : defaultNavLinks;
      ...
      <div className="max-w-7xl mx-auto h-full px-4 sm:px-6">
        <div className="flex items-center justify-between h-full gap-3">
          ...
          <div className="flex items-center shrink-0">
            ...
            <NavLink
              to="/"
              className="text-lg -ml-2 sm:text-2xl font-bold tracking-tight text-primary-dark whitespace-nowrap"
            >
              RaktSeva
            </NavLink>
          </div>
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-1 p-1.5 sm:p-2 text-xs sm:text-sm lg:text-base font-medium tracking-tight transition-colors duration-200 ${
                      isActive ? "text-primary-green" : "text-primary-dark/50"
                    }`
                  }
                >
                  <Icon size={18} />
